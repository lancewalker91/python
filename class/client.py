...
g_add_msg_url = "http://test.gn100.com/message.plan.add"
g_ws_url = "ws://test.gn100.com:8001/message.plan.ws"
...
class Client:
    ...
    def start(self):
        while True:
            ...
            a = self.s.cookies.get("uid_test")
            if not a:
                yield gen.sleep(3)
                self.log_func("login fails for {}".format(self.user))
                continue
            self.user_id = int(a)
            self.token = self.s.cookies.get("token_test")
...
if "__main__" == __name__:
    ...
    define("user", default="18810000000", type=str, help="phone")
    define("passwd", default="18810000000", type=str, help="passwd")
