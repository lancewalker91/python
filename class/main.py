...
class ControlHandler(WebSocketHandler):
    help_str = """usage:
        ...
        text plan_id phone token repeat start end live_second content -- 发文本信息（发送时间为starth到end之间的随机数之后，发repeat次,live_second代表距离视频开始的时间）
        texts plan_id phone num repeat start end live_second content -- 发文本信息（发送从phone开始，连着num个手机号,live_second同上）
        answer[ answer] -- 设置答题的答案（没有参数表示不让答题,可以用分号分隔提交多个答案，逗号分隔表示多选（随机答一个））
        """
    members = []
    def log_write(buf):
        ...
        try:
            ...
            if "add" == items[0] and len(items) >= 3:
                ...
                for i in range(num):
                    ...
                    c = client.Client(user+i,"000000", plan_id, ControlHandler.log_write)
                    c.start()
            else:
...
if "__main__" == __name__:
    ...
    define("port", default=9101, type=int, help="port")
    parse_command_line()
    main(options.port)
